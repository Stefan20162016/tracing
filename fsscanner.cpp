...
notes:
- g++ -O3 -std=c++17 -pthread fsscanner.cpp -o fsscanner && time sudo ./fsscanner 8 / # 1.3 sec
...
- fsscanner_atomic.cpp checks an atomic variable if some thread is still running and might find another directory
...
#define NR_GLOBAL_FILENAME_MUTEXES 16 // also works surprisingly well with 1
...
public:
    ...
    void operator()(){
        try{
            if(start_with_path != ""){
                ...
                #ifndef DONTSLEEP                
                std::this_thread::sleep_for(std::chrono::milliseconds(3 + worker_id%8));     
                #endif                    
                working(); 
            }
    ...
    void working(){
        ...
        #endif
        #define RETRY_COUNT 7 // a few or a few dozen works equally well (for me)
    ...
    check_again:
        ...
        goto check_again; // after returning from do_linear_descent() check if there is more work
